...
type ModelRecord = {
  ...
  quantization?: {
    ...
    options: { id: string; label: string; quality: string; estimatedSize?: string; warning: boolean; warningText?: string }[];
  };
};
...
function ModelEditDialog({ model, onClose, onComplete }: { model: ModelRecord; onClose: () => void; onComplete: () => Promise<void> }) {
  const options = model.quantization?.options || [];
  const [level, setLevel] = useState(options[0]?.id || "");
  ...
  async function quantize() {
    ...
    const warning = selected.warningText ? `\n\n${selected.warningText}` : "";
    if (!window.confirm(`Create a new ${selected.id} copy of ${model.name}? The original model will not be changed.${warning}`)) return;
    setBusy(true);
    setError("");
    setStatus("Quantizing locally. This can take several minutes; keep this page open.");
    try {
      const response = await fetch("/api/models/quantize", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ modelId: model.id, level: selected.id }) });
      const payload = await response.json() as { filename?: string; message?: string; error?: string; detail?: string };
      if (!response.ok || !payload.filename) throw new Error(payload.detail || payload.error || "Local quantization failed");
      await onComplete();
      setStatus(`${payload.message || "Quantized copy saved"}: ${payload.filename}`);
    } catch (cause) {
      setStatus("");
      setError(cause instanceof Error ? cause.message : "Local quantization failed");
    } finally {
      setBusy(false);
    }
  }

  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (!busy && event.currentTarget === event.target) onClose(); }}><section className="settings-dialog model-edit-dialog" role="dialog" aria-modal="true" aria-labelledby="model-edit-title"><div className="dialog-heading"><div><span className="card-kicker">Edit model</span><h2 id="model-edit-title">{model.name}</h2></div><button className="icon-button" onClick={onClose} aria-label="Close model editor" disabled={busy}>×</button></div><div className="model-edit-summary"><span className={`model-glyph ${model.type}`}>{model.type === "language" ? "✦" : "◇"}</span><div><strong>{model.filename}</strong><small>{[model.size, model.quantization?.currentLevel && `Current: ${model.quantization.currentLevel}`].filter(Boolean).join(" · ")}</small></div></div><div className="quantization-section"><span className="card-kicker">Quantization</span>{model.quantization?.supported ? <><label className="control-field"><span>Compression level</span><select value={level} onChange={(event) => { setLevel(event.target.value); setStatus(""); setError(""); }} disabled={busy}>{options.map((option) => <option value={option.id} key={option.id}>{option.label}</option>)}</select></label>{selected && <div className="quantization-estimate"><div><span>Estimated resulting size</span><strong>{selected.estimatedSize || "Estimate unavailable"}</strong></div><div><span>Expected quality</span><strong>{selected.quality}</strong></div></div>}{selected?.warning && <div className="quality-warning" role="alert"><strong>Quality warning</strong><p>{selected.warningText || "This level is likely to cause significant quality degradation."}</p></div>}<p className="field-note">The result is saved as a new model. The original file is never overwritten.</p></> : <div className="unavailable-note"><strong>Quantization unavailable</strong><p>{model.quantization?.unavailableReason || "This model cannot be quantized by the local runtime."}</p></div>}</div>{(busy || status || error) && <div className={`job-status ${error ? "error" : ""}`} aria-live="polite"><span className={`pulse ${error ? "amber" : ""}`} /><span>{error || status}</span></div>}<div className="dialog-actions"><button className="button button-secondary-light" onClick={onClose} disabled={busy}>Close</button>{model.quantization?.supported && <button className="button button-primary" onClick={() => void quantize()} disabled={!selected || busy}>{busy ? "Quantizing locally…" : "Create quantized copy"}</button>}</div></section></div>;
}

function Models({ system, chooseModel, onRefresh }: { system: SystemSnapshot | null; chooseModel: (model: ModelRecord) => void; onRefresh: () => Promise<void> }) {
