...
function Chat({ models, selectedId, onSelect }: { models: ModelRecord[]; selectedId: string | null; onSelect: (id: string) => void }) {
  ...
  const [turns, setTurns] = useState<{ role: "user" | "assistant"; text: string; reasoning?: string }[]>([]);
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const payload = await response.json() as { answer?: string; reasoning?: string; error?: string; detail?: string };
      if (!response.ok || !payload.answer) throw new Error(payload.detail || payload.error || "Local generation failed");
      setTurns((current) => [...current, { role: "assistant", text: payload.answer!, reasoning: payload.reasoning }]);
    } catch (cause) {
